...
%   产生的新的模型诞生的特征向量，即新的观察值（状态向量）以及获取即使奖励
    ...
    clear metricstime
    %加上成功文件个数的特征
    succstate = success_fileNum*ones(success_fileNum,1);      %列向量n*1
    new_State1 = cat(2,new_State1,succstate);
    new_State1          %可清除 clear new_State1
    % 若成功的小于100，则补零
    ...
    new_State2      %可清除 clear new_State2
    %deadfeature
%{
    new_State3 = cat(2,new_State2,deadfeature');   %100*22,100*1 =100*23  死块加复杂度
    disp(new_State3);
    %}
    cd(oldpath)
    cd
  ...
  %{
    ...
    new_State = [new_State3 blockfeature];      %100*23,100*127=100*150
        %}
    %成功文件数量
%     succstate = [success_fileNum*ones(success_fileNum,1);zeros(neededState,1)];      %列向量n*1
%     new_State = cat(2,new_State2,succstate);
    ...
    %oldstate n*150;
    %归一化处理
    new_State = Min_Max_Normalization(new_State);
    ...
    save(fullfile(successPath,'Rewardreport.mat'),'totalState','reward','Action','oldstate','new_State2','new_State','-append')
    save(dqncfg.CACHDATA_FILE,'totalState')
    clear totalState
    clear Feature
    
    %%  清除变量
    vars = {'newParam','cfg_newParam','cfg_new','reportsneofile','successfile','new_State1','new_State2'};
    clear(vars{:})
